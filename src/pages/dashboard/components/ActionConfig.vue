...
// 战斗房（只影响战斗技能等级，对生活的加成按等级总和折算进 combatHouseLevel）
const COMBAT_HOUSE_ROOMS = new Set(["dojo", "gym", "archery_range", "armory", "library", "mystical_study", "dining_room"])
  ...
  try {
    ...
    if (data.skills && data.version === 1) {
      ...
      for (const [roomHrid, level] of Object.entries(data.houses || {})) {
        ...
        if (COMBAT_HOUSE_ROOMS.has(room!) && typeof level === "number") combatHouseTotal += level as number
      }
    ...
    const config: ActionConfig = {
      ...
      achievementBuffMap,
      shrineBuffMap,
      combatHouseLevel: combatHouseTotal
    }
    ...
    if (shouldMerge && mergeTargetIndex !== undefined) {
      ...
      config.combatHouseLevel = Math.max(config.combatHouseLevel ?? 0, existing.combatHouseLevel ?? 0)
